        ...
        <div class="row">
            <div class="card">
              <div class="card-header">
                <h5 class="title">Etudiant</h5>
              </div>
              <div class="card-body">
                <form method="POST" action="etudiant.php">
                <div class="form-group">
                        <label>Entrer le Matricule</label>
                        <input type="number" name="code"class="form-control" placeholder="Matricule">
                      </div>
                                                <br>
...
<input type="submit" name="saved" value="Enregistrer" class="btn btn-fill btn-primary">
                                     ...
                                     <?php
   extract ($_POST);
    if (isset($code) and !empty($saved)){

      $salle = $fil." ".$an;
$resul =  "Cet Enregistrement existe deja ";
$rq = $bdd->query("SELECT * FROM etudiant WHERE matricule='$code' AND filiere='$salle'  LIMIT 1");
$user = $rq->fetch();
  if ($user) {
...
echo $resul;
      ...
      $id = '0';   
$req = $bdd->prepare('INSERT INTO etudiant (id, matricule, filiere) VALUES(:id, :matricule, :filiere)');
$req->execute(array(
'id' => $id, 
'matricule'  =>$code,
'filiere' =>$salle
